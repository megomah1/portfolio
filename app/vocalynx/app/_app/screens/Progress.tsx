...
export default function Progress({ store }: { store: Store }) {
  ...
  if (sessions.length === 0) {
    return (
      <div className="flex h-full flex-col items-center justify-center px-8 text-center">
        <h1 className="font-display text-2xl font-semibold">No data yet</h1>
  ...
  return (
    <div className="flex h-full flex-col overflow-y-auto px-5 pb-6">
      <div className="pt-5">
        ...
        <p className="mt-1 text-sm text-ink-2">Your vocal-health report.</p>
      </div>
      ...
      <p className="mt-6 rounded-xl bg-accent-tint/60 px-4 py-3 text-xs leading-relaxed text-ink-2">
        Reports like this are the kind of summary a speech therapist can review at a glance —
        what you practised and where your voice is trending — instead of scrubbing through raw
        recordings.
      </p>
    </div>
  );
